...
#            print("ELEMENT %s" % (etree.tostring(element), ))
        ...
        if self.state == "connected":
            mechs = self.xmlroot.findall(
                './stream:features/sasl:mechanisms/sasl:mechanism', namespaces=self.__nsmap__)
            for auth in mechs:
...
#           print(etree.tostring(self.xmlroot))
            challenges = self.xmlroot.findall(
                './sasl:challenge', namespaces=self.__nsmap__)
...
#               logger.debug(cstring)
                ...
                for pair in re.findall(b'(\w+\s*=\s*(?:(?:"[^"]+")|(?:[^,]+)))',cstring):
                    key,value=[x.strip()
                               for x in pair.decode('ascii').split('=', 1)]
...
#                   resp['username']=self.username.encode('ascii')
                    ...
                    resp['digest-uri']=b'xmpp/' + \
                        self.server.encode('ascii') # example.com'
                    A1=C([H(C([resp['username'],resp['realm'],self.password.encode('ascii')])),resp[
                         'nonce'],resp['cnonce']])
                    A2=C([b'AUTHENTICATE',resp['digest-uri']])
                    response= HH(C([HH(A1).encode('ascii'),resp['nonce'],resp['nc'],resp[
                                 'cnonce'],resp['qop'],HH(A2).encode('ascii')]))
...
#                   logger.debug(sasl_data)
                    n.text = base64.b64encode(
                        sasl_data.encode('ascii')).decode('ascii')
                    self.sendxmlobj(n)
                    self.state = "digest-md5"
        ...
        if self.state == "sasl" or self.state == "digest-md5":
            sasl = self.xmlroot.findall(
                './sasl:success', namespaces=self.__nsmap__)
            if len(sasl) == 1:
                self.xmlroot = None
                self.element = None
                self.elements = []
            ...
            features = self.xmlroot.findall(
                './stream:features', namespaces=self.__nsmap__)
            for i in features:
                for j in i:
                    logger.debug(j.tag)
            ...
            binds = self.xmlroot.findall(
                './jabber:iq/bind:bind/bind:jid', namespaces=self.__nsmap__)
            for bind in binds:
                n = etree.Element('iq', attrib={
                    'type' :  'set',
                    'id' : 'bind_1'})
                s = etree.SubElement(
                    n, 'session', attrib = { 'xmlns' : "urn:ietf:params:xml:ns:xmpp-session" })
                self.sendxmlobj(n)
                self.state = "session"
                # cleanup './jabber:iq/bind:bind/bind:jid' below via
                # ./jabber:iq
            ...
            iqs = self.xmlroot.findall(
                './jabber:iq', namespaces=self.__nsmap__)
            for iq in iqs:
                self.xmlroot.remove(iq)

            for channel in self.channels:
                to = "%s@%s/%s" % (channel, self.muc,
                                   self.username + '-' + self.resource)
                n = etree.Element('presence', attrib={
                    'to' : to })
                p = etree.Element(
                    'x', attrib = { 'xmlns' : "http://jabber.org/protocol/muc" })
                self.sendxmlobj(n)
                logger.info("trying to join %s" % to)
            self.state = "join"
        elif self.state == "join":
            presences = self.xmlroot.findall(
                './jabber:presence', namespaces=self.__nsmap__)
            for presence in presences:
                ...
                if to == me and channel in self.channels:
                    muis = presence.findall(
                        './mucuser:x/mucuser:item[@jid="%s"]' % to, namespaces=self.__nsmap__)
                    self.joined = self.joined + len(muis)
                    errors = presence.iterfind(
                        './error', namespaces=self.__nsmap__)
                    for error in errors:
                        logger.warn("could not join %s\n%s" %
                                    (to, etree.tostring(error).decode('ascii')))
        ...
        if self.state == "online":
            ...
            files = self.xmlroot.findall(
                './jabber:message/jabber:body/dionaea:dionaea/dionaea:file', namespaces=self.__nsmap__)
            for i in files:
                xmlobj = i
                ...
                if md5_hash == my_hash:
                    fileobj = tempfile.NamedTemporaryFile(delete=False, prefix='xmpp-', suffix=g_dionaea.config(
            ...
            messages = self.xmlroot.findall(
                './jabber:message', namespaces=self.__nsmap__)
            ...
            for iq in iqs:
                if 'id' in iq.attrib:
                    ...
                    if id in self.ids:
                        ...
                        # remove iqs which are replies to our requests, like
                        # ping
                        del self.ids[id]
                        self.xmlroot.remove(iq)
...
class logxmpp(ihandler):
    def __init__(self, server, port, username, password, resource, muc, config):
        ...
        self.resource = resource
        self.username = username.split('@')[0]
        self.client = xmppclient(server=server, port=port, username=username,
                                 password=password, resource=resource, muc=muc, channels=list(self.config.keys()))
        ...
        for to in self.config:
            ...
            for r in self.config[to]['pcre']:
                ...
                try:
                    ...
                    func = getattr(
                        self, "serialize_incident_" + handler_name, None)
                except:
                    func = None
