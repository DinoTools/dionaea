...
class surfidshandler(ihandler):
    ...
    def _handle_incident_dionaea_connection_tcp_accept(self, icd):
        con=icd.con
        r = self.stmt_attack_add(
            0, con.remote.host, con.remote.port, con.local.host, con.local.port, None, self.sensor_type)
        attackid = r[0][0]
        ...
        self.stmt_detail_add(
            attackid, con.local.host, DT_PROTOCOL_NAME, con.protocol)
    ...
    def _handle_incident_dionaea_connection_tcp_connect(self, icd):
        ...
        r = self.stmt_attack_add(
            0, con.remote.hostname, con.remote.port, con.local.host, con.local.port, None, self.sensor_type)
        attackid = r[0][0]
        self.attacks[con] = attackid
        logger.info("connect connection to %s/%s:%i from %s:%i (id=%i)" %
                    (con.remote.host, con.remote.hostname, con.remote.port, con.local.host, con.local.port, attackid))
...
#        self.stmt_detail_add(attackid, con.local.host,
        ...
        self.stmt_attack_update_severity(
            attackid, AS_DEFINITLY_MALICIOUS_CONNECTION)
        self.stmt_detail_add(
            attackid, con.local.host, DT_EMULATION_PROFILE, icd.profile)

    def _handle_incident_dionaea_download_offer(self, icd):
        con=icd.con
        attackid = self.attacks[con]
        logger.info("offer for attackid %i" % attackid)
        self.stmt_detail_add_offer(
            con.remote.host, con.local.host, icd.url, self.sensor_type)

    def _handle_incident_dionaea_download_complete_hash(self, icd):
        con=icd.con
        attackid = self.attacks[con]
        logger.info("complete for attackid %i" % attackid)
        self.stmt_detail_add_download(
            con.remote.host, con.local.host, icd.url, icd.md5hash, self.sensor_type)
    ...
    def _handle_incident_dionaea_service_shell_listen(self, icd):
        ...
        self.stmt_detail_add(
            attackid, con.local.host, DT_SHELLCODE_ACTION, "bindshell://"+str(icd.port) )

    def _handle_incident_dionaea_service_shell_connect(self, icd):
        con=icd.con
        attackid = self.attacks[con]
        logger.info("connect shell for attackid %i" % attackid)
        self.stmt_detail_add(attackid, con.local.host, DT_SHELLCODE_ACTION,
                             "connectbackshell://"+str(icd.host)+":"+str(icd.port) )
...
#        logger.info("WATCHME: %s" % str(vuln))
        ...
        self.stmt_detail_add(
            attackid, con.local.host, DT_DCERPC_REQUEST, icd.uuid + ":" + str(icd.opnum))
