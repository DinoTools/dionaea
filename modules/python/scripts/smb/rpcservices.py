...
#	syntax = UUID('8a885d04-1ceb-11c9-9fe8-08002b104860').hex
	...
	def processrequest(cls, service, con, opnum, p):
		if opnum in cls.ops:
			...
			#TODO add support for p.PacketFlags == 3 if needed
			if opname == "WritePrinter":
				if p.PacketFlags == 1:
					opname = "WritePrinter_firstfrag"
				if p.PacketFlags == 0 or p.PacketFlags == 2:
					opname = "WritePrinter_midfrag"
...
class spoolss(RPCService):
	...
	class DOC_INFO_1:
		...
		def __init__(self, p):
			...
			if isinstance(self.__packer,ndrlib.Packer):
				...
				print("DocName %s OutputFile %s" %(self.DocName,self.OutputFile))
	...
	@classmethod
	def handle_WritePrinter_firstfrag(cls, p):
		...
		Buf = x.unpack_raw(4000)

		r = ndrlib.Packer()
		# Nothing need to return as it is the first fragment
		return None

	@classmethod
	def handle_WritePrinter_midfrag(cls, p):
		#WritePrinter Function		
		#
		#http://msdn.microsoft.com/en-us/library/dd145226%28v=VS.85%29.aspx
		#
		...
		r = ndrlib.Packer()
		# For the Middle fragment, nothing need to return
		# TODO For the Last fragment, we need to determine to overall buffer received to contruct the correct response with LPDWORD pcWritten
		return None
