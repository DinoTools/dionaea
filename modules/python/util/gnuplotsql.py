...
# SPDX-FileCopyrightText: 2011-2018 PhiBo (DinoTools)
#
...
def get_ranges_from_db(cursor):
    ...
    stop = datetime.datetime(stop.year,stop.month,1)+datetime.timedelta(
        days=calendar.monthrange(stop.year,stop.month)[1])-datetime.timedelta(seconds=1)
...
def make_directories(ranges, path_destination):
    ...
    for r in ranges:
        if r[0] == 'month':
            path = os.path.join(
                path_destination, r[1].strftime("%Y"), r[1].strftime("%m"))
#			print(path)
