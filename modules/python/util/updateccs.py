...
#
# SPDX-FileCopyrightText: 2003 Graham King
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--type-col', help='the column containing the cc type', required=True)
    parser.add_argument(
        '--num-col', help='the column containing the cc number', required=True)
    ...
    cursor = dbh.cursor()
    query = "UPDATE {:s} SET {:s}=CAST(gencc({:s}) AS INTEGER)".format(
        args.table,args.num_col,args.type_col)
    print(query)
